...
{
    // A gradient as a strip of per-band quads, flat-filled from one solid UV sample. Shared by
    // UIGradationRect (canvas, rect in px) and SpriteGradationFeed (world, rect in units) — the
    // geometry is frame-agnostic.
    public static class UIGradationMeshGen
    {
        private static readonly List<(float Pos, Color Color)> _points = new();

        public static void Populate(
            Gradient gradient, bool horizontal, Rect rect, Color color, Sprite sprite, MeshBuilder mb)
        {
            Assert.IsTrue(_points.Count == 0, "Points should be empty");

            MergeGradientPoints(gradient, _points);
...
#if UNITY_EDITOR
        // What Populate needs of a gradient, for an authoring-time [ValidateInput].
        public static bool Validate(Gradient gradient, ref string errorMessage)
        {
            ...
            if (gradient.alphaKeys.Length < 2)
            {
            ...
            if (gradient.mode is GradientMode.Fixed)
            {
